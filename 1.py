                ...
                if events[i]._author_id != user._id:
                    if user._id in event[i]._users.keys():
                        if event[i]._users[user._id] == 0:
                            ...
                            text_event1 = ft.Text(print_event(event))
                            ...
                            click_12 = ft.ElevatedButton("Не внoсить событие в ваш календарь",
                                                         on_click=next1)
                            row6 = ft.Column([text_welcome1, text_event1, click_11, click_12])
                            page.add(row6)
                        elif event._users[user._id] == 2:
                            text_welcome2 = ft.Text(f"Вас исключили из списка участников событи:")
                            text_event2 = ft.Text(print_event(event))
                            click_13 = ft.ElevatedButton("Далее", on_click=next1)
                            row7 = ft.Row([text_welcome2, text_event2, click_13])
                            page.add(row7)
            page.add(row5)
        else:
